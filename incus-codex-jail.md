...
# [Incus System-Container Jail for the Codex Coding Agent](https://weisser-zwerg.dev/posts/incus-codex-jail/)

Published by Weisser Zwerg Blog on February 18, 2026.

Reducing 'Supply Chain Attack' Risk from AI Coding Agents (and Shadow AI).

1. Rationale

  1. Incus (formerly LXD)
  2. Different Readers and Different Levels of Prior Knowledge

2. Outline: Controlling Access Without Ruining the Workflow
...
6. Appendix
  ...
  1. agent-jail-profile.yaml
  2. project-orientation Skill
  3. MCP Servers

    1. Context7: Version-Aware Library Documentation (Hosted)
    2. Sequential Thinking: Structured Planning (via a Skill)
    3. When not to use an MCP Server

  4. The Better Full Replacement: agent-jail-recursive-profile.yaml

    1. Deltas In agent-jail-helpers.sh
    2. agent-jail-recursive-profile.yaml
...
I publish the `project-orientation` skill below in the appendix.
...
Not every tool benefits from being agent-controlled via MCP. If a tool’s main job is to *select and package local code/context*[1], I prefer to run it as a human-in-the-loop CLI step so I can review exactly what gets included before it ever reaches the model.
...
If you really insist on using an MCP-based context server, have a look at [Claude Context](https://github.com/zilliztech/claude-context). Despite the name, it isn’t Claude-specific-the documentation includes instructions for using it with Codex. ↩︎

## Feedback

 Have you written a [response](https://indieweb.org/responses) to this? Let me know the URL via [telegraph](https://telegraph.p3k.io/send-a-webmention).
